class BenefitsController < ApplicationController
  ...
  # POST /benefits.json
  def create
    if params.has_key?("benefit")
      Benefit.create(benefits_params(params["benefit"]))
    else
      params["benefits"].each do |benefit|
        if benefit["description"] != "" || benefit["solution_id"] != ""
          Benefit.create(benefits_params(benefit))
        end
      end
    end
    ...
    #respond_to do |format|
      #if @benefit.save
        #format.html { redirect_to @benefit, notice: 'Beneficio was successfully created.' }
  ...
  def benefits_params(my_params)
    my_params.permit(:description, :solution_id)
  end
  ...
  private
    ...
    def benefit_params
      params.require(:benefit).permit(:solution_id, :description, :validated)
    end
end
